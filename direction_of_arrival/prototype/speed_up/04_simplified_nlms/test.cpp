...
//	NLMS Variables
	...
	// Number of channels in each sub-array
	int num_chan[cluster_number] = {5,5,4,4,1};
	int det_chan[cluster_number];
	...
	for (int n = 0; n < N_chan; n++)
	{
		...
		if (n == n_index)
		{
			det_chan[c_index] = temp - 1;
			n_index += num_chan[c_index];
			++c_index;
		}
	}

	NLMS filter_1(num_chan[0], filter_order, step_size, alpha, threshold, tiny, det_chan[0]);
	NLMS filter_2(num_chan[1], filter_order, step_size, alpha, threshold, tiny, det_chan[1]);
	NLMS filter_3(num_chan[2], filter_order, step_size, alpha, threshold, tiny, det_chan[2]);
	NLMS filter_4(num_chan[3], filter_order, step_size, alpha, threshold, tiny, det_chan[3]);
	NLMS filter_raw(num_chan[4], filter_order, step_size, alpha, threshold, tiny, det_chan[4]);
	double* pointer;
