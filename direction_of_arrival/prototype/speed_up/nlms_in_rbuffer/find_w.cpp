...
// <@@>
  int k = i_start; for (int i = 0; i < n_omegas; ++i) list_bins[i] = k++;
...
void perform_opt(opt_params* params, gsl_matrix_complex** cps,
  ...
  gsl_vector_scale(&x0.vector, -1.001/(343.0*gsl_blas_dnrm2(&x0.vector)));
...
// Sets noise to identity
	...
	position = gsl_matrix_row(params->R, mic1);
	for (int i = 0; i < 3; i++)
	{
		gsl_vector_complex_set(
			slowness,
			i,
			{gsl_vector_get(&position.vector,i),0}
		);
	}
	gsl_vector_complex_scale(
		slowness,
		{-1.0/(343.001*gsl_blas_dnrm2(&position.vector)),0.0}
	);
