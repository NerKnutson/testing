  ...
  if (infile.is_open())
  {
    while (getline(infile, line))
    {
      ...
      if(m_x.Full())
      {
        ...
        for (int j = 0; j < N_chan; j++)
        {
//          if (m_error[j]*m_error[j] > 4*m_var[j])
//          {
//            std::cout << "TRIGGERED";
//          }
          m_var[j] = alpha*m_var[j] + (1-alpha)*m_error[j]*m_error[j];
          std::cout << std::setw(15) << m_error[j];
        }
