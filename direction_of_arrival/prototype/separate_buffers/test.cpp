...
//	Ring Buffer
	RingBuffer buff_hist(N_chan,history_size);
	RingBuffer buff_shot(N_chan,window_size);
...
if (infile.is_open())
{
	while (getline(infile, line))
	{
		...
		if (deci.ProcessData(array))
		{
			...
			if (!buff_hist.Full())
				buff_hist.Put((double*)deci.m_data_out);
			else if (buff_shot.Size() <= tukey_length)
				buff_shot.Put((double*)deci.m_data_out);
			else if (!start_waiting && n_filter.IsRecording())
 			{
				buff_shot.Put((double*)deci.m_data_out);
				buff_shot.Get((double*)array);
				buff_hist.Put((double*)array);
			}
			else if (start_waiting)
				buff_shot.Put((double*)deci.m_data_out);

			counter++;
		}

		if (buff_shot.Size() > tukey_length and !start_waiting and n_filter.IsTriggered(which_channel))
		{
		...
		// Arrays to dump in are made
			...
			buff_hist.Dump(history_size,history,NULL);
			buff_shot.Dump(window_size,shot,NULL);
			buff_shot.Reset();
